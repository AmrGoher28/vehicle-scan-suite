  ...
  try {
    ...
    if (confirmedItems.length > 0) {
      const rows = confirmedItems.map((item) => ({
        ...
        damage_x_percent: item.damage_x_percent ?? null,
        damage_y_percent: item.damage_y_percent ?? null,
      }));
